...
public class BluetoothService {
	...
	private static final UUID MY_UUID = UUID
			.fromString("00001101-0000-1000-8000-00805F9B34FB");  // 임베디드 <---> 안드로이드
	...
	private synchronized void setState(int state) {
		...
		if (state==1) {
			Message msg = mHandler.obtainMessage(Fragment_Menu1.CONNECT_FAIL);
	...
	public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
        ...
        Message msg = mHandler.obtainMessage(Fragment_Menu1.MESSAGE_DEVICE_NAME);
        Bundle bundle = new Bundle();
        bundle.putString(Fragment_Menu1.DEVICE_NAME, device.getName());
        msg.setData(bundle);
        mHandler.sendMessage(msg);
		...
		public void run() {
			...
			while (true) {
				try {
					...
					mHandler.obtainMessage(Fragment_Menu1.MESSAGE_READ, bytes, -1, buffer)
                    .sendToTarget();
				} catch (IOException e) {
